...
// Khóa trạng thái cục bộ bảo vệ chống lặp lệnh bất đồng bộ
let isTransitioning = false;
let isResolvingVote = false;
let localTimerInterval = null;
// Danh sách các vai trò thụ động ban đêm (không cần chọn mục tiêu)
...
// =========================================================================
// BẮT ĐẦU OBJECT STATEMACHINE (DÒNG NÀY BẮT BUỘC PHẢI CÓ)
// =========================================================================
...
syncPhaseTimer(endTime, duration) {
    ...
    const updateVisuals = () => {
        ...
        // Tự động kích hoạt chuyển pha khi hết giờ (Dành cho Quản trò/Host)
        if (remainingMs <= 0) {
...
async handleTimerExpiration() {
    ...
    if (phase === "night") {
        ...
        if (trialStage === "mayor_election") {
            ...
            // Hết giờ biện hộ -> Chuyển sang Bỏ phiếu phán quyết
...
async transitionToNight() {
    ...
    try {
        ...
        if (snap.exists()) {
            ...
            // Khởi tạo lại lượt đi đêm
            Object.entries(players).forEach(([playerId, player]) => {
                updates[`rooms/${Net.roomId}/players/${playerId}/targetSelection`] = null;
                
                // Người đã chết, mất kết nối, hoặc vai trò thụ động -> Tự động xong lượt
...
async transitionToDay() {
    ...
    try {
        ...
        // Gửi mật thư vào Hòm Thư Mailbox
        ...
        // Công bố danh sách tử vong sáng nay
        let announcement = "";
...
async resolveVotingOutcome() {
    ...
    try {
        ...
        // Tính phiếu bầu (Phiếu Trưởng Làng có trọng số 2)
        ...
        runGavelStrikeAnimation(decisionText, async () => {
            try {
                ...
                if (executeTarget) {
                    ...
                    // XỬ LÝ VAI TRÒ ĐẶC BIỆT: Gã Hề (Clown) bị treo cổ -> Thắng Đơn Lập!
                    ...
                    // XỬ LÝ VAI TRÒ ĐẶC BIỆT: Thợ Săn (Hunter) bị treo cổ ban ngày -> Bật Modal bắn trả thù!
